...
export async function getUserUsage(userId: string): Promise<UserUsageData> {
    ...
    if (shouldResetUsage(usageData.usageResetAt)) {
        const resetUsage: UsageStats = {
            ...
            fixWithPr: 0,
            pitchDecks: 0,
            databaseConnections: usageData.usage.databaseConnections, // Don't reset connection count
            databaseQueries: 0,
        };
...
export async function getRemainingUsage(userId: string): Promise<{
    ...
    if (userData.isPro) {
        return {
            ...
            remaining: {
                ...
                databaseConnections: "unlimited",
                databaseQueries: "unlimited",
            },
    ...
    const remaining: Record<keyof UsageStats, number | "unlimited"> = {
        ...
        pitchDecks: Math.max(0, FREE_TIER_LIMITS.pitchDecks - userData.usage.pitchDecks),
        databaseConnections: Math.max(0, FREE_TIER_LIMITS.databaseConnections - userData.usage.databaseConnections),
        databaseQueries: Math.max(0, FREE_TIER_LIMITS.databaseQueries - userData.usage.databaseQueries),
    };
